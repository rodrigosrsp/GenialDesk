...
hbb_name = 'genialdesk' + ('.exe' if windows else '')
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/genialdesk.exe')
    os.chdir('../..')
    if os.path.exists('./genialdesk_portable.exe'):
        os.replace('./target/release/rustdesk-portable-packer.exe',
                   './genialdesk_portable.exe')
    else:
        os.rename('./target/release/rustdesk-portable-packer.exe',
                  './genialdesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/genialdesk_portable.exe')
    os.rename('./genialdesk_portable.exe', f'./genialdesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/genialdesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/genialdesk.exe target/release/GenialDesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\genialdesk.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/GenialDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/genialdesk-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/genialdesk-{version}-win7-install.exe ../..')
